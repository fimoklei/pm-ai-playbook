...
- **Intent specs**: Glob `.kiro/specs/*/intent-spec.md`. If matches exist, note them — these are pre-discovery exploration outputs (produced by the `kiro-intent` / `intent-explorer` skill) that already capture user-need, problem statement, and outcomes for in-progress features.
...
**Intent-spec shortcut**: If an `intent-spec.md` exists for a feature matching the user's request (detected in Step 1), read it before asking any questions. The intent-spec already covers: who has the problem (Who and why), desired outcome, problem statement, and verification criteria. **Skip questions 1, 2, and 7 entirely** — they are answered. Focus only on questions 3, 4, 5, 6 (Boundary candidates, Out of boundary, Existing vs new, Upstream / downstream), since intent-spec.md is product/why-oriented and does not cover boundary or routing concerns. Acknowledge to the user which intent-spec was loaded so the skipping is transparent.
