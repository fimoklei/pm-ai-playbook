...
|---|---|---|
| Payment API behavior | `pnpm --filter payment-service test:api` | Required for endpoint or worker changes |
| Adapter changes | `pnpm --filter payment-service test:adapters` | Required when touching `src/adapters/` |
| Type changes | `pnpm --filter payment-service typecheck` | Required for public payment types |

## Definition of Done

- Relevant tests pass.
- Typecheck passes where applicable.
- No processor contract changed without contract tests.
...
| Change type | Required command | Notes |
|---|---|---|
| Route behavior | `pnpm --filter user-service test` | Required before final response |
| Type changes | `pnpm --filter user-service typecheck` | Required for API types |
