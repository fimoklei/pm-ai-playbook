...
2. Run Codex review as final gate:
   - Check for docs-only diff first:
     - Run `git diff --name-only <base-ref>...HEAD`.
     - If every changed path matches the regex `(^|.*/)(README|LICENSE)[^/]*$|\.md$|^docs/`, the diff is docs-only.
     - Print one line to the user: "Docs-only diff detected — skipping review."
     - Skip the rest of step 2 and continue to step 3.
...
- If `codex` is not on PATH, stop with a clear error — do not silently skip the review gate.
- If `codex review` exits non-zero, stop and report the failure — do not push.
